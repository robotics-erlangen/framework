...
function CatchBall:_createBallObstacles(path, robotDir, currentBall, predictedBall)
	...
	local corridorStartLeft = corridorEndLeft + corridorLeftDir:perpendicular():setLength(-(self._robot.shootRadius + predictedBall.radius)*0.6 + radiusCompensation)
	...
	local corridorStartRight = corridorEndRight + corridorRightDir:perpendicular():setLength((self._robot.shootRadius + predictedBall.radius)*0.6 + radiusCompensation)

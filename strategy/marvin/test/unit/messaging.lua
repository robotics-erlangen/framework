...
return function()
	...
	agent1send.moveDest("all", Vector.create(0,0))
	Messaging.deliverMessages()
	assert(not agent1inbox.moveDest()[agent1:robot()], "broadcasts shall not be received by the sender")

	--agent1send(agent2:robot(), "moveDest", Vector.create(0,0))
	agent3send.moveDest("all", Vector.create(0,0))
	Messaging.deliverMessages()
	agent2 = AgentAttacker.create(World.FriendlyRobots[2])

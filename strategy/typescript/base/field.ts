...
function intersectionsRayDefenseArea_2017 (pos: Position, dir: RelativePosition, extraDistance: number = 0,
	...
	if (radius < 0) {
		throw "extraDistance must not be smaller than -G.DefenseRadius";
	}
...
function defenseIntersectionByWay_2017 (way: number, extraDistance: number = 0, friendly: boolean): Position {
	...
	let radius = G.DefenseRadius + extraDistance;
	if (radius < 0) {
		throw "extraDistance must not be smaller than -G.DefenseRadius: "+String(extraDistance);
	}
	...
	if (way < 0) {
		throw "way is out of bounds ("+String(way)+", "+String(extraDistance)+", "+String(friendly);
	}

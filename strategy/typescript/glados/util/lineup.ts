...
function generateLineup (lineStart: Position, lineupDir: number) {
	...
	while (friendlyIndex <= World.FriendlyRobots.length) {
		...
		if (opp && opp.pos.distanceTo(intendedPos) < mindist) {
			...
			let task = new MoveToPos(<any>pseudoagent, intendedPos, viewDir);
			task.run();
...
Entrypoints.add("Lineup/Friendly Left", function() { generateLineup(fleft, Math.PI/2); return true; });
Entrypoints.add("Lineup/Friendly Right", function() { generateLineup(fright, Math.PI/2); return true; });
Entrypoints.add("Lineup/Opponent Left", function() { generateLineup(oleft, -Math.PI/2); return true; });
Entrypoints.add("Lineup/Opponent Right", function() { generateLineup(oright, -Math.PI/2); return true; });
Entrypoints.add("Lineup/Middle Left", function() { generateLineup(mleft, Math.PI/2); return true; });
Entrypoints.add("Lineup/Middle Right", function() { generateLineup(mright, Math.PI/2); return true; });

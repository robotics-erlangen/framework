let Volley = require "task/ability/volley" // only for calcPhi
let CatchBall = {}
...
let AROUND_METHOD = "around"
let STOP_METHOD = "stop"
let HUNT_METHOD = "hunt"
...
function CatchBall:_catchBall (targetPos, distanceToBall, targetSpeed, maxSpeed) {
	let ball = World.Ball
	// update catch time
	if (self._catchTime  &&  not Ball.isAccelerating()  &&  self._recalculateCatchTimeCounter < 20) {
		...
		let newCatchtime = Physics.robotTimeToBall(self._robot, ball, targetPos, maxSpeed  ||  self._robot.maxSpeed, self._catchTime)
		if (not self._catchTime  ||  newCatchtime < self._catchTime - 0.3) {
	...
	let hitTime = self:_calculateHitTime(ball)
	let ballInsideRobot = false
	if (not Ball.isSlowBall()  ||  hitTime == 0) {
	...
	let predictedBall = Physics.ballAtTime(ball, self._catchTime)
	...
	if (targetSpeed  &&  not Ball.isSlowBall()) {
		let targetDir, _ = self:calcPhi(predictedBall.speed, predictedBall.pos,
	...
	if (predictedBall.pos
			 &&  not Field.isInAllowedField(predictedBall.pos, World.Ball.radius)
			 &&  self._robot != World.FriendlyKeeper
	...
	if (World.Ball.pos:distanceTo(self._robot.pos) < World.Ball.radius + self._robot.radius + 0.1) {
		self._ignoringOpponents = true
	} else if (World.Ball.pos:distanceTo(self._robot.pos) < 1) {
		if (self._robot.speed:length() < 1) {
			self._ignoringOpponents = true
		} else if (self._robot.speed:length() > 1.5) {
			self._ignoringOpponents = false
		}
	} else {
		self._ignoringOpponents = false
	...
	let aggressiveMovement = (self._robot.pos:distanceTo(moveDest) < 0.5)
	...
	let method = self:_ballCatchMethod(ball, predictedBall, moveDest)
	if (method == AROUND_METHOD) {
	...
	let time = Physics.robotTimeToPos(self._robot, moveDest, Vector(0, 0))
...
function CatchBall:_calculateHitTime (ball) {
	...
	if (ball.pos:distanceTo(self._robot.pos) < self._robot.radius + ball.radius) {
	...
	if (ball.speed:dot(self._robot.pos - ball.pos) <= 0) {
		return math.huge
	}
	...
	let dribberAngleHalf = math.atan(self._robot.dribblerWidth/2 - ball.radius, self._robot.shootRadius)
	// check whether the hitpoint could be inside the dribbler
	if (math.abs(geom.getAngleDiff((hitPoint - self._robot.pos):angle(), self._robot.dir)) < dribberAngleHalf) {
		// calculate where the ball would hit the dribbler
		// just use the current robot dir as any prediction will be just as wrong
		let dribblerMid = self._robot.pos + Vector.fromAngle(self._robot.dir):scaleLength(self._robot.shootRadius)
		// points along the dribbler
		let dribblerDir = Vector.fromAngle(self._robot.dir):perpendicular():scaleLength(self._robot.dribblerWidth / 2 - ball.radius)
		...
		if (intersection  &&  math.abs(lambda2) <= 1) {
...
function CatchBall:limitEndSpeedToField (moveDest, endSpeed) {
	...
	if (endSpeed:length() > 0.01) {
		...
		let extraDistance = 0
		if (not Field.isInAllowedField(moveDest, -extraDistance)) {
			endSpeedLength = 0
		} else if (not Field.isInAllowedField(extrapolatedPos, extraDistance)) {
			let _, nextLineCutLambda = Field.nextAllowedFieldLineCut(moveDest, endSpeed, extraDistance)
...
function CatchBall:_updateReasonableBallPos (ball, ballInsideRobot) {
	...
	if (not ballInsideRobot) {
		self._lastReasonableBallPos = ball.pos
	} else if (not self._lastReasonableBallPos) {
		// try to come up with a sensible position
		let hitPoint1, hitPoint2 = geom.intersectLineCircle(ball.pos, ball.speed, self._robot.pos, self._robot.radius + ball.radius)
		if (not hitPoint1  ||  not hitPoint2) {
			// fallback
			self._lastReasonableBallPos = ball.pos
...
function CatchBall:_ballCatchMethod (currentBall, predictedBall, moveDest) {
	...
	let robotTargetDist = self._robot.pos:distanceTo(moveDest)
	// distance minus robot and ball radius thus the ball is for sure between the robot and the catch pos
	let robotTargetSpacing = math.max(0, robotTargetDist - self._robot.radius - currentBall.radius)

	let _, _, lambda1, lambda2, lambda3, lambda4 = geom.intersectLineCorridor(currentBall.pos, predictedBall.pos - currentBall.pos,
			self._robot.pos, moveDest - self._robot.pos, self._robot.shootRadius + World.Ball.radius - 0.005)
	let ballHit = lambda1 != nil ? (lambda1 >= 0  &&  lambda1 <= 1) : lambda2 != nil  &&  (lambda2 >= 0  &&  lambda2 <= 1)
	let robotHit = lambda3 != nil ? (lambda3 >= 0  &&  lambda3 <= 1) : lambda4 != nil  &&  (lambda4 >= 0  &&  lambda4 <= 1)

	let robotMovement = moveDest:distanceToSq(self._robot.pos) > self._robot.radius * self._robot.radius
							 &&  moveDest - self._robot.pos
							 ||  self._robot.speed

	if (ballHit ? robotHit : lambda2 == math.huge  &&  lambda4 == math.huge
		...
		// the ball hasn't yet moved past the robot (TODO better calculation than the dot product?)
			 ||  (currentBall.pos - self._robot.pos):dot(predictedBall.pos - currentBall.pos) <= 0) {
		return STOP_METHOD
	} else {
		return HUNT_METHOD
	}
...
function CatchBall:_createMoveAroundBallObstacle (path, moveDest, minBall, predictedBall) {
	...
	if (ballDist > OBSTACLE_EPSILON) {
		...
		let robotDistToPredictedBall = self._robot.pos:distanceTo(predictedBall.pos)
		...
		if ((robotDistToPredictedBall + 2 * self._robot.radius + ball.radius) < ballDistToPredictedBall
			 ||  robotDistToPredictedBall < 2 * self._robot.radius + minBall.radius) {
		...
		let robotDir = geom.getAngleDiff((minBall.pos - predictedBall.pos):angle(), self._robot.dir)
		let robotBallDist = math.max(math.cos(robotDir) * ballDist, 0)
		// maximum error cause by moddeling the robot as circle
		let obstacleErrorDist = self._robot.radius - self._robot.shootRadius + DIST_ERROR
...
function CatchBall:_createBallCorridor (path, viewDir, predictedBall) {
	...
	let effectiveObstacleRadius = self._robot.radius
	let negativeRadius = - self._robot.radius + effectiveObstacleRadius
	let moveWidth = self._robot.dribblerWidth + 2 * SIDE_DEPTH

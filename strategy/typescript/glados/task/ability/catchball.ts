...
function CatchBall:init () {
	this._lastReasonableBallPos = nil
	this._catchTime = nil
...
function CatchBall:_catchBall (targetPos, distanceToBall, targetSpeed, maxSpeed) {
	let ball = World.Ball
	// update catch time
	if (this._catchTime && not Ball.isAccelerating() && this._recalculateCatchTimeCounter < 20) {
		...
		let newCatchtime = Physics.robotTimeToBall(this._robot, ball, targetPos, maxSpeed || this._robot.maxSpeed, this._catchTime)
		if (not this._catchTime || newCatchtime < this._catchTime - 0.3) {
	...
	if (ballInsideRobot || predictedBall.pos:isNan() || predictedBall.speed:isNan()) {
		predictedBall = { pos = this._lastReasonableBallPos, speed = new Vector(0, 0), maxSpeed = ball.maxSpeed, radius = ball.radius }
	}
	...
	if (targetSpeed && not Ball.isSlowBall()) {
		let targetDir, _ = this.calcPhi(predictedBall.speed, predictedBall.pos,
    ...
    PathHelper.setObstacleParam(this._robot, "disableOpponentPrediction", aggressiveMovement)
    PathHelper.setObstacleParam(this._robot, "ignoreOpponentRobots", this._ignoringOpponents)
	...
	let time = Physics.robotTimeToPos(this._robot, moveDest, new Vector(0, 0))
	this._send.moveDest("all", moveDest)
	this._send.attackPosition("all", predictedBall.pos)
...
function CatchBall:_calculateHitTime (ball) {
	...
	let dribberAngleHalf = Math.atan(this._robot.dribblerWidth/2 - ball.radius, this._robot.shootRadius)
...
function CatchBall:_updateReasonableBallPos (ball, ballInsideRobot) {
	...
	} else if (not this._lastReasonableBallPos) {

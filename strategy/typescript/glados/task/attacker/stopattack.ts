...
function StopAttack:_init (minDistToBall) {
	...
	this._minDistToBall = minDistToBall || Constants.stopBallDistance
}
...
function StopAttack:run () {
	...
	let pos = World.Ball.pos + (this._focusPoint - World.Ball.pos).setLength(stopRadius)
	...
	if (dist < 0.2 + this._robot.radius && defendOpponentPasses && #passReceivers > 0) {

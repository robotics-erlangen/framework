...
function Duel:run () {
	...
	if (not this._opposer) {
		this._opposer = Ball.firstRobotAtBall(World.OpponentRobots)
	}

	// notify all that we are duelling
	let distToOpp = this._opposer ? this._robot.pos.distanceTo(this._opposer.pos) : Infinity
	...
	if (this._defendedOpponentMessageSent) {
		this._send.defendedOpponent("all", this._opposer)
	}


	if (this._opposer && this._blockingBall && Robot.hadBall(this._robot, 0)) {
		this._contest()
...
function Duel:_contestRotate () {
	...
	let toOpponentDir = this._opposer.pos - this._robot.pos
...
function Duel:_contest () {
	...
	if (this._robot.dir > 0 && this._robot.dir < Math.PI && World.Ball.pos.y > 0.2
			 &&  not Robot.hadBall(this._opposer, 0)) {
...
function Duel:_moveToNearBlock (closestOpponentRobot) {
	...
	let baseDir = (this._futureBall - World.Geometry.FriendlyGoal).angle()
	let oppViewDir = (this._futureBall - closestOpponentRobot.pos).angle()
...
function Duel:_moveToBall () {
	...
	if (intersectionDefenseArea) {
		...
		moveDest = this._futureBall + (intersectionDefenseArea - this._futureBall).setLength(this._robot.shootRadius + World.Ball.radius)
		...
		moveDest = this._futureBall + (this._robot.pos - this._futureBall).setLength(this._robot.shootRadius + World.Ball.radius)
	}
	...
	if (this._blockingBall) {
		if (closestOpponentRobot) {
			...
			moveDest = this._futureBall + (World.Geometry.FriendlyGoal - this._futureBall).setLength(
	...
	// send the position of the ball
	this._send.attackPosition("all", this._futureBall)
}

...
let SAFE_GOAL_MID = G.FriendlyGoal - new Vector(0, 0.05)
...
function ShootoutKeeper:run () {
	...
	PathHelper.setObstacleParam(this._robot, "ignoreBall", not ballCloserToGoal)
	if (ballCloserToGoal) {
	...
	if (ballSpeed.y < 0) {
		...
		let predictedGoallinePoint = geom.intersectLineLine(G.FriendlyGoal, new Vector(1, 0), pos, dir).x
		...
		let interceptPos = this._robot.pos.orthogonalProjection(pos, pos+dir)
...
let leftFriendlyCorner = new Vector(-G.FieldWidthHalf, -G.FieldHeightHalf)
let rightFriendlyCorner = new Vector(G.FieldWidthHalf, -G.FieldHeightHalf)

// assume chips crossing this line might cross the goal line
let leftNearBasePoint = new Vector(-G.FieldWidthHalf, G.FieldHeightHalf-CHIP_GOAL_LINE_DIST)
let rightNearBasePoint = new Vector(G.FieldWidthHalf, G.FieldHeightHalf-CHIP_GOAL_LINE_DIST)

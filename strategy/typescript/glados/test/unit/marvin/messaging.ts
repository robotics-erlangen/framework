...
let agentStub = function (robotStub) {
	let agent = {}
	agent.robot = function() return robotStub }
	return agent
}

let fold = function (tables) {
...
test("Messaging", function()
	...
	let agent1send, agent1inbox = messaging:registerAgent(agent1)
	let agent2 = agentStub(robotStub(2))
	let agent2send, _ = messaging:registerAgent(agent2)
	let agent3 = agentStub(robotStub(3))
	let agent3send, _ = messaging:registerAgent(agent3)
	let trainerSend, trainerInbox = messaging:registerTrainer()

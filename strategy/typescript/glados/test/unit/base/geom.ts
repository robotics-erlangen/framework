...
export class BaseGeom extends UnitTest {
	...
	private testIntersectCircleCircle() {
		...
		}
		{
			...
			this.assert_eq_eps(ret2!.distanceToSq(new Vector(1.75, Math.sqrt(15) / 4)), 0, EPS);
			this.assert_eq_eps(l2!.distanceToSq(new Vector(1.75, -Math.sqrt(15) / 4)), 0, EPS);
		}
		{
			let [ret3, l3] = geom.intersectCircleCircle(new Vector(0, 0), 2, new Vector(0, Math.sqrt(3)), 1);
			this.assert_eq_eps(ret3!.distanceToSq(new Vector(1, Math.sqrt(3))), 0, EPS);
			this.assert_eq_eps(l3!.distanceToSq(new Vector(-1, Math.sqrt(3))), 0, EPS);
		}
		{
			let [ret4, l4] = geom.intersectCircleCircle(new Vector(0, 0), 2, new Vector(2, 2), 1);
			this.assert_eq_eps(ret4!.distanceToSq(new Vector(1.0 / 8 * (11 + Math.sqrt(7)), 1.0 / 8 * (11 - Math.sqrt(7)))), 0, EPS);
			this.assert_eq_eps(l4!.distanceToSq(new Vector(1.0 / 8 * (11 - Math.sqrt(7)), 1.0 / 8 * (11 + Math.sqrt(7)))), 0, EPS);
		}
		{
	...
	private testBoundRect() {
		// right
		this.assert_vector_eq(geom.boundRect(new Vector(-1, -1), new Vector(2, 0), new Vector(1, 1)), new Vector(1, 0));
		this.assert_vector_eq(geom.boundRect(new Vector(-1, -1), new Vector(0.5, 0), new Vector(1, 1)), new Vector(0.5, 0));
		...
		// top
		this.assert_vector_eq(geom.boundRect(new Vector(-1, -1), new Vector(0, -2), new Vector(1, 1)), new Vector(0, -1));
		this.assert_vector_eq(geom.boundRect(new Vector(-1, -1), new Vector(0, -0.5), new Vector(1, 1)), new Vector(0, -0.5));
		...
		// other
		this.assert_vector_eq(geom.boundRect(new Vector(0, 0), new Vector(123, 321), new Vector(0, 0)), new Vector(0, 0));
	}
	...
	private testIntersectLineCircle() {
		...
		this.assert_eq(geom.intersectLineCircle(new Vector(-2, -2), new Vector(1, 0), new Vector(0, 0), 1).length, 0);
	}
	...
	private testTriangleArea() {
		...
		for (let i = 1; i < 90; ++i) {
			...
			this.assert_eq_eps(area, a * b * 0.5, EPS);
		}
	}

	private testCheckTriangleOrientation() {
		this.assert_eq(geom.checkTriangleOrientation(new Vector(-1, 0), new Vector(0, 1), new Vector(1, 0)), -1);
		this.assert_eq(geom.checkTriangleOrientation(new Vector(-1, 0), new Vector(0, 0), new Vector(1, 0)), 0);
		this.assert_eq(geom.checkTriangleOrientation(new Vector(1, 0), new Vector(0, 1), new Vector(-1, 0)), 1);
	}

	private testQuadrangleArea() {
		// Squares
		this.assert_eq_eps(geom.calcQuadrangleArea(new Vector(0, 0), new Vector(1, 0), new Vector(1, 1), new Vector(0, 1)), 1, EPS);
		this.assert_eq_eps(geom.calcQuadrangleArea(new Vector(0, 0), new Vector(0, 1), new Vector(1, 1), new Vector(1, 0)), 1, EPS);
		this.assert_eq_eps(geom.calcQuadrangleArea(new Vector(0, 0), new Vector(3, 4), new Vector(-1, 7), new Vector(-4, 3)), 25, EPS);
		// Rhombi
		for (let i = 1; i < 50; ++i) {
			this.assert_eq_eps(geom.calcQuadrangleArea(new Vector(0, 0), new Vector(5, i), new Vector(10, 0), new Vector(5, -i)), 10 * i, EPS);
		}
		// Parallelogram
		for (let i = 0; i < 500; ++i) {
			this.assert_eq_eps(geom.calcQuadrangleArea(new Vector(0, 0), new Vector(5, 0), new Vector(5 + i, 2), new Vector(i, 2)), 10, EPS);
		}
	}
	...
	private testGeomCenter() {
		...
		this.assert_vector_eq_eps(geom.center(circle), new Vector(0, 0), EPS);
		// square
		let square = [new Vector(0, 0), new Vector(2, 0), new Vector(2, 2), new Vector(0, 2)];
		this.assert_vector_eq_eps(geom.center(square), new Vector(1, 1), EPS);
	}
	...
	private testNormalizeAngle() {
		this.assert_eq(geom.normalizeAngle(0), 0);
		this.assert_eq(geom.normalizeAngle(-1), -1);
		this.assert_eq(geom.normalizeAngle(1), 1);

		this.assert_eq_eps(geom.normalizeAngle(4), 4 - 2 * Math.PI, 0.001);
		this.assert_eq_eps(geom.normalizeAngle(-4), -4 + 2 * Math.PI, 0.001);
	...
	private testNormalizeAnglePositive() {
		this.assert_eq(geom.normalizeAnglePositive(0), 0);
		this.assert_eq(geom.normalizeAnglePositive(1), 1);
		this.assert_eq(geom.normalizeAnglePositive(4), 4);

		this.assert_eq_eps(geom.normalizeAnglePositive(7), 7 - 2 * Math.PI, 0.001);
		this.assert_eq_eps(geom.normalizeAnglePositive(-1), -1 + 2 * Math.PI, 0.001);
	...
	private testGetAngleDiff() {
		...
		for (let i = 0; i < 1000; ++i) {
			...
			this.assert_le(res, Math.PI);
			this.assert_le(-Math.PI, res);
		}

		this.assert_eq_eps(geom.getAngleDiff(-0.5, 0.5), 1, EPS);
	}

	private testBisectingAngle() {
		let pi = Math.PI;
		let piHalf = Math.PI / 2;
		this.assert_eq(geom.bisectingAngle(0, 0), 0);
		this.assert_eq(geom.bisectingAngle(pi, pi), pi);
		this.assert_eq(geom.bisectingAngle(-pi, -pi), -pi);
		this.assert_eq(geom.bisectingAngle(piHalf, piHalf), piHalf);
		this.assert_eq(geom.bisectingAngle(-piHalf, -piHalf), -piHalf);

		this.assert_eq(geom.bisectingAngle(pi, -pi), pi);
		this.assert_eq(geom.bisectingAngle(piHalf, -piHalf), 0);

		this.assert_eq(geom.bisectingAngle((pi / 4), -(pi / 4)), 0);
		this.assert_eq(geom.bisectingAngle((pi * 3 / 4), -(pi * 3 / 4)), pi);
		this.assert_eq(geom.bisectingAngle((pi * 3 / 4), (pi / 4)), (pi / 2));
		this.assert_eq(geom.bisectingAngle(-(pi * 3 / 4), -(pi / 4)), -(pi / 2));

		this.assert_eq(geom.bisectingAngle((7 * pi / 9), -(7 * pi / 9)), pi);
		this.assert_eq(geom.bisectingAngle(-(7 * pi / 9), (7 * pi / 9)), pi);

		this.assert_eq(geom.bisectingAngle(pi / 6, pi / 8), pi * 7 / 48);
		this.assert_eq(geom.bisectingAngle(pi / 8, pi / 6), pi * 7 / 48);
	...
	private testInscribedAngle() {
		...
		this.assert_vector_eq_eps(c1, c2, EPS);
		this.assert_vector_eq_eps(c1, new Vector(1, 0), EPS);
		this.assert_eq_eps(r, 1, EPS);
		[c1, c2, r] = geom.inscribedAngle(new Vector(0, 0), new Vector(6, 8), Math.PI / 2);
		this.assert_vector_eq_eps(c1, c2, EPS);
		this.assert_vector_eq_eps(c1, new Vector(3, 4), EPS);
		this.assert_eq_eps(r, 5, EPS);
	...
	private testAngleBound() {
		...
		this.assert_eq(geom.angleBound(1.0 / 2 * Math.PI, Math.PI, 3.0 / 2 * Math.PI), Math.PI);
		// easy scenario: angle is left boundary
		this.assert_eq(geom.angleBound(1.0 / 2 * Math.PI, 1.0 / 2 * Math.PI, 3.0 / 2 * Math.PI), 1.0 / 2 * Math.PI);
		// easy scenario: angle is right boundary
		this.assert_eq(geom.angleBound(1.0 / 2 * Math.PI, 3.0 / 2 * Math.PI, 3.0 / 2 * Math.PI), 3.0 / 2 * Math.PI);

		// medium scenario: normal bound would be correct
		this.assert_eq(geom.angleBound(1.0 / 2 * Math.PI, 7.0 / 4 * Math.PI, 3.0 / 2 * Math.PI), 3.0 / 2 * Math.PI);
		this.assert_eq(geom.angleBound(1.0 / 2 * Math.PI, 1.0 / 4 * Math.PI, 3.0 / 2 * Math.PI), 1.0 / 2 * Math.PI);

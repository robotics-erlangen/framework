...
std::optional<QString> LogFileReader::logUIDFromStatus(const Status status) {
...
QString LogFileReader::logUID()
{
    ...
    if (!maybeId) {
        return "This log does not contain a log UID. To create one, just use the Logcutter on this log.";
    }

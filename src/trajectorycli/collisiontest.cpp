...
static RobotState updateOpponent(const RobotState &opp, const Scenario &s, const RobotState &friendlyRobot)
{
    if (s.testType == CollisionTestType::BLOCKED_LINE) {
        ...
        auto evil = AlphaTimeTrajectory::find(opp, targetState, ACCELERATION, MAX_SPEED, 0, EndSpeed::FAST);
        if (evil) {
            return evil.value().stateAtTime(0.01f);
        }
    }
...
static bool opponentCloseToRobot(const Scenario &s)
{
    ...
    auto direct = AlphaTimeTrajectory::find(s.ownStart, targetState, ACCELERATION, MAX_SPEED, 0, EndSpeed::EXACT);
    if (!direct) {
        return true;
    }
